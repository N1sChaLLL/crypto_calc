...
class Calculator:
    ...
    def verify_all(self):
        """Compare our implementations with trusted Python operations."""
        tests = 20
        lines = [
            ...
            "Reference used:",
            "  math.gcd() for GCD",
            "  Python pow(a, e, m) for modular exponentiation",
            "  Python pow(a, -1, m) for modular inverse",
            "  Python %, + and * for basic modular operations",
            "",
            f"Running {tests} random test cases...",
            ""
        ]
        ...
        for _ in range(tests):
            ...
            mine = divides(a, b)
            reference = (b % a == 0) if a != 0 else (b == 0)

            if mine == reference:
                div_passed += 1

        lines.append(f"Divisibility:             {div_passed}/{tests} passed")
        ...
        lines.append(f"GCD vs math.gcd():       {gcd_passed}/{tests} passed")
        ...
        for _ in range(tests):
            ...
            if g == math.gcd(a, b) and a * x + b * y == g:
                ext_passed += 1

        lines.append(f"Extended Euclid:         {ext_passed}/{tests} passed")
        ...
        for _ in range(tests):
            ...
            if modular_add(a, b, m) == (a + b) % m:
                add_passed += 1

        lines.append(f"Modular addition:        {add_passed}/{tests} passed")
        ...
        for _ in range(tests):
            ...
            if modular_multiply(a, b, m) == (a * b) % m:
                mul_passed += 1

        lines.append(f"Modular multiplication:  {mul_passed}/{tests} passed")

        # Repeated squaring vs Python's built-in pow()
        power_passed = 0
        for _ in range(tests):
            a = random.randint(-10**6, 10**6)
            ...
            mine = modular_power_repeated_squaring(a, e, m)
            reference = pow(a, e, m)
        ...
        lines.append(
            f"Repeated squaring vs pow(): {power_passed}/{tests} passed"
        )

        # Modular inverse vs Python's pow(a, -1, m)
        inverse_tests = 0
        inverse_passed = 0
        ...
        lines.append(
            f"Modular inverse vs pow(): {inverse_passed}/{inverse_tests} passed"
        )
        ...
        all_passed = (
            div_passed == tests
            and gcd_passed == tests
            and ext_passed == tests
            and add_passed == tests
            and mul_passed == tests
            and power_passed == tests
            and inverse_passed == inverse_tests
        )

        if all_passed:
            lines.append("OVERALL RESULT: PASS")
            lines.append(
                "All implementations matched their trusted references."
            )
        else:
            lines.append("OVERALL RESULT: CHECK FAILED")
            lines.append(
                "At least one implementation did not match its reference."
            )
